...
static bool check_function(struct pci_root_bus *bus, struct pci_address *address) {
...
static bool check_device(struct pci_root_bus *bus, struct pci_address *address) {
...
void pci_write_config_space(struct pci_address *address, uint32_t offset, uint32_t value) {
    pci_write(address, offset, value);
}
...
static bool handle_root_bridge_io_protocol(EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL *root_bridge_io_protocol) {
...
bool pci_initialize(void) {
    ...
    if (gBS->LocateProtocol(&root_bridge_io_protocol_guid, NULL, (void **)&root_bridge_io_protocol) != EFI_SUCCESS) {
        if (acpi_full_init() != EFI_SUCCESS) {
            return false;
        }
    ...
    } else if (!handle_root_bridge_io_protocol(root_bridge_io_protocol)) {
        return false;
    }
